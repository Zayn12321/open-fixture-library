/**
 * @fileoverview
 * There are some JSON files (also in dependencies) that need to be imported
 * synchronously, but we don't want to use the `--experimental-json-modules`
 * command-line flag for every `node` command.
 * Thus, we require them all here in this CommonJS module and reexport them for
 * use in ECMAScript modules.
 *
 * @see https://nodejs.org/docs/latest-v14.x/api/esm.html#esm_experimental_json_modules
 */

module.exports.colorNameList = require(`color-name-list`);
